...
class VoltageSourceAdapter:
    ...
    def configure(self, voltage_range, compliance_current=None, nplc=1.0):
        if self.kind is None:
            raise ValueError('Selected device cannot source voltage in this configuration.')
        if self.kind == 'mock':
            return
        if self.resource is None:
            raise RuntimeError('Instrument resource handle unavailable.')
        try:
    ...
    def set_voltage(self, voltage):
        ...
        try:
            ...
            raise RuntimeError(f'Failed to program source-drain voltage: {exc}')
...
class GateSourceAdapter:
    ...
    def configure(self, voltage_range, compliance_current=None, nplc=1.0):
        if self.kind is None:
            raise ValueError('Selected gate device cannot source voltage.')
        if self.kind == 'mock':
            return
        if self.resource is None:
            raise RuntimeError('Instrument resource handle unavailable.')
    ...
    def set_voltage(self, voltage):
        ...
        try:
            ...
            raise RuntimeError(f'Failed to program gate voltage: {exc}')
        if not self.output_enabled:
            try:
                self.resource.write("OUTP ON")
            except Exception:
...
class ProbeMeterAdapter:
    ...
    def configure(self, voltage_range=None, nplc=1.0):
        if self.kind is None:
            raise ValueError('Selected probe meter cannot measure voltage.')
        if self.kind == 'mock':
            return
        if self.kind == 'smu':
            if self.resource is None:
...
class FourProbeFET(QWidget):
    ...
    def start_measurement(self):
        ...
        try:
            self.sd_adapter = VoltageSourceAdapter(self.sd_device)
        ...
        try:
            self.probe_adapter = ProbeMeterAdapter(self.probe_device)
        ...
        try:
            self.gate_adapter = GateSourceAdapter(self.gate_device)
